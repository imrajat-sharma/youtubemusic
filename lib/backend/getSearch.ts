...
import { getClient, listMapper, parsePublished, streamMapper } from '@/lib/backend/utils';
...
export async function getSearch(params: { q: string; f?: string }) {
  const { q, f } = params;
  ...
  if (f === 'relevance' || f === 'upload_date' || f === 'view_count') {
    ...
    if (f === 'upload_date') filters.upload_date = 'all';
    if (f === 'view_count') filters.prioritize = 'popularity';
  ...
  if (f === 'upload_date') {
    contents.sort((a, b) => {
      const timeA = a.is(YTNodes.Video)
        ? parsePublished(a.as(YTNodes.Video).published?.toString() || '')
        : 0;
      const timeB = b.is(YTNodes.Video)
        ? parsePublished(b.as(YTNodes.Video).published?.toString() || '')
        : 0;
      return timeB - timeA;
    });
  }
